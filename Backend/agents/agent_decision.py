...
class AgentConfig:
    ...
    # Decision model (configured via config.py)
    DECISION_MODEL = "gemini-3.1-flash-lite-preview"

    # Vision model for image analysis (configured via config.py)
    ...
    7. MEDICAL_PATHOLOGY_AGENT - For unified blood and tissue pathology classification:
...
def create_agent_graph():
    ...
    def has_explicit_harmful_intent(text: str) -> bool:
        """Lightweight harmful intent check used when relaxing image-intent guardrails."""
        harmful_patterns = [
            r"\b(suicide|self[-\s]?harm|kill myself|end my life)\b",
            r"\b(make|build|create).*(bomb|weapon|explosive)\b",
            r"\b(hack|malware|ransomware|exploit)\b",
            r"\b(illegal drugs?|meth|cocaine|heroin|fentanyl)\b",
        ]
        lowered = (text or "").lower()
        return any(re.search(pattern, lowered) for pattern in harmful_patterns)
    ...
    def analyze_input(state: AgentState) -> AgentState:
        ...
        if input_text:
            ...
            if medical_image_intent and not has_explicit_harmful_intent(input_text):
                is_allowed, message = True, input_text
            else:
    ...
    def route_to_agent(state: AgentState) -> Dict:
        ...
        if has_image and (image_type in ["BLOOD SMEAR", "PATHOLOGY SLIDE", "PATHOLOGY", "BLOOD", "TISSUE"] or "pathology" in str(current_input).lower()):
            ...
            return {"agent_state": updated_state, "next": "MEDICAL_PATHOLOGY_AGENT"}
    ...
    def run_rag_agent(state: AgentState) -> AgentState:
        ...
        if hasattr(response_content, 'content'):
            ...
            response_text = extract_text_content(response_content.content)
        else:
            # If it's already a string
            response_text = extract_text_content(response_content)
    ...
    def run_brain_tumor_agent(state: AgentState) -> AgentState:
        ...
        if classification_result.get("success"):
            ...
            response = AIMessage(content=f"Could not process the uploaded brain MRI image for tumor classification. Details: {error_message}")
    ...
    def run_medical_pathology_agent(state: AgentState) -> AgentState:
        """Handle blood/tissue pathology image analysis with 4-class classification.
        ...
        print(f"Selected agent: MEDICAL_PATHOLOGY_AGENT")

        # Use the unified blood and tissue pathology classifier
        ...
        if not success or error:
            error_message = error or "Unknown model or image issue"
            response = AIMessage(
                content=(
                    "❌ Could not process the uploaded pathology image. "
    ...
    workflow.add_node("MEDICAL_PATHOLOGY_AGENT", run_medical_pathology_agent)
    ...
    workflow.add_conditional_edges(
        ...
        lambda x: x["next"],
        {
            ...
            "MEDICAL_PATHOLOGY_AGENT": "MEDICAL_PATHOLOGY_AGENT",
    ...
    workflow.add_edge("MEDICAL_PATHOLOGY_AGENT", "check_validation")
