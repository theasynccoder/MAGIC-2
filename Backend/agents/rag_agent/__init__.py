    ...
    def ingest_file(self, document_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                if "WinError 1314" not in error_text:
                    raise
                self.logger.warning(
                    "Docling parser failed due to Windows privilege/symlink issue. "
                    "Falling back to plain-text PDF parsing (no image extraction)."
                )
                parsed_document = self.doc_parser.parse_document_fallback(document_path)
                images = []
            ...
            if used_fallback_parser:
                formatted_document = parsed_document
            else:
